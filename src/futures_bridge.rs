// Forked from https://github.com/rustwasm/wasm-bindgen/blob/master/crates/futures/src/lib.rs
//
...
//!    `Future<Output = JsValue, Error = JsValue>`. This Rust future will resolve
//!    or reject with the value coming out of the `Promise`.
//!
//! 2. [**`future_to_promise`**](./fn.future_to_promise.html)
//!
//!    Converts a Rust `Future<Output = JsValue, Error = JsValue>` into a
...
//!             Ok(42)
//!         })
//!         // And then convert the `Output` and `Error` into `JsValue`.
//!         .map(|result| {
//!             JsValue::from(result)
//!         })
//!         .map_err(|error| {
...
//!         });
//!
//!     // Convert the `Future<Output = JsValue, Error = JsValue>` into a JavaScript
//!     // `Promise`!
...
use futures::future;
use futures::prelude::*;
use futures::task::{Poll, Spawn, Waker};
...
impl Future for JsFuture {
    ...
    fn poll(self: Pin<&mut Self>, _waker: &Waker) -> Poll<Result<JsValue, JsValue>> {
        ...
        if let Ok(Some(val)) = self.resolved.try_recv() {
            drop(self.callbacks.take());
            return Poll::Ready(Ok(val.into()));
        }
        if let Ok(Some(val)) = self.rejected.try_recv() {
            drop(self.callbacks.take());
...
fn _future_to_promise(future: Box<Future<Output = JsValue>>) -> Promise {
    ...
    struct Package {
        ...
        spawn: RefCell<Box<Future<Output = JsValue>>>,
...
{
    future_to_promise(future.map(|()| JsValue::undefined()));
}
